...
import { USER_NAV_ITEMS } from '@/lib/nav-config';
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild size="lg" className="data-[slot=sidebar-menu-button]:px-3 data-[slot=sidebar-menu-button]:py-2.5">
              <Link href="/user" onClick={closeMobile} className="flex items-center gap-3">
                <UserIcon className="h-5 w-5 text-sidebar-primary" size={20} />
                <span className="font-semibold text-sidebar-foreground">User Console</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
      ...
      <SidebarContent>
        <SidebarMenu>
          {USER_NAV_ITEMS.map((item) => {
            const Icon = ICON_MAP[item.id] ?? ChartLineUpIcon;
            const isActive =
              pathname === item.href ||
