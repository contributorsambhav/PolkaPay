...
type TooltipPayloadItem = { name?: string; value?: unknown; dataKey?: string; payload?: Record<string, unknown>; fill?: string; color?: string };
    ...
    <div className={cn('border-border/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl', className)}>
      ...
      <div className="grid gap-1.5">
        {payload.map((item: TooltipPayloadItem, index: number) => {
          ...
          return (
            <div key={String(item.dataKey ?? index)} className={cn('[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5', indicator === 'dot' && 'items-center')}>
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div className={cn('flex flex-1 justify-between leading-none', nestLabel ? 'items-end' : 'items-center')}>
                    ...
                    {item.value != null && <span className="text-foreground font-mono font-medium tabular-nums">{String(item.value)}</span>}
                  </div>
                </>
              )}
            </div>
...
type LegendPayloadItem = { value?: unknown; dataKey?: string; color?: string };
  ...
  return (
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item: LegendPayloadItem) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        return (
          <div key={String(item.value ?? item.dataKey ?? '')} className={cn('[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3')}>
